...
- name: Start the container runtime
  vars:
    ...
    block_explorer_image_tag: 0.1.2
...
Generates or transfers a client certificate for the committer sidecar mTLS connection when the sidecar requires mTLS. Selects OpenSSL, `cryptogen`, or Fabric CA based on `organization`; see the `organization` option for the selection rules. Then publishes Kubernetes Secret material when Kubernetes mode is enabled.
...
> Transfer a cryptogen-generated client certificate for Block Explorer

Transfers the client TLS key, certificate, and CA certificate generated by `cryptogen` for the Block Explorer peer identity named by `organization.peer.name`.
...
> Enroll Block Explorer with Fabric CA for client TLS

Enrolls the Block Explorer peer identity through Fabric CA to generate client TLS material under `block_explorer_remote_config_dir` on the target host. Uses `actual_host` in the CSR SAN list and writes fetched CA artifacts under `fetched_artifacts_dir`. Requires the organization definition to reference a Fabric CA host and peer identity.
