...
Fabric CA, CA databases, orderer, committer, PostgreSQL, load generator, Prometheus, and Grafana use OpenShift task paths. Ansible still runs from the control node, but inventory hosts represent OpenShift resources rather than SSH machines. Node Exporter is not deployed on OpenShift; node-level metrics come from the OpenShift platform monitoring stack.
