...
Orderer, committer, PostgreSQL, load generator, Prometheus, and Grafana use OpenShift task paths. `cryptogen` runs on the control node and writes artifacts below `cryptogen_artifacts_dir`. Node Exporter is not deployed on OpenShift; node-level metrics come from the OpenShift platform monitoring stack.
...
- 1 load generator.
- Monitoring with PostgreSQL exporter, Prometheus, Grafana, and OpenShift platform monitoring for node-level metrics.
