...
| 12     | [Using Ansible Vault for Secrets](./12-using-ansible-vault-for-secrets.md) | 25 min    | Encrypting credentials instead of writing them in the open               |
| 13     | [Troubleshooting](./13-troubleshooting.md)                                 | reference | How to read failures and recover from them                               |

The thirteen lessons fall into four stages:
...
flowchart TB
  ...
  subgraph CHANGE["Stage 4 — Make it yours"]
    ...
    L11 --> L12[12. Ansible Vault]
  end

  LEARN --> RUN
  RUN --> READ
  READ --> CHANGE
  CHANGE --> L13[13. Troubleshooting<br/>reference, dip in as needed]
```
